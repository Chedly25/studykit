  ...
  if (showWizard || profiles.length === 0) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-8 animate-fade-in">
        <h1 className="text-2xl font-bold text-[var(--text-heading)] mb-6">Create Exam Profile</h1>
        <ExamProfileWizard />
      </div>
    )
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-[var(--text-heading)]">Exam Profiles</h1>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {isActive && (
                      <span className="text-xs px-2 py-0.5 rounded-full bg-[var(--accent-bg)] text-[var(--accent-text)]">Active</span>
                    )}
                  </div>
                  <div className="text-sm text-[var(--text-muted)] mt-1">
                    {bp?.label} &middot; {daysLeft} days left &middot; Target: {p.weeklyTargetHours}h/week
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  {!isActive && (
                  ...
                  <button
                    onClick={() => {
                      if (confirm(`Delete "${p.name}" and all associated data?`)) {
                        deleteProfile(p.id)
                      }
                    }}
