/**
 * CRFPA Commentaire d'arrêt Trainer page.
 * Decision picker (by chamber) → editor (intro + plan) → results.
 */
...
export default function CommentaireCoach() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)]">
      <Helmet>
        <title>Commentaire d'arrêt — Coach CRFPA | StudiesKit</title>
        <meta
          name="description"
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-y-auto">
          <div className="flex items-center gap-3 px-4 py-3 border-b border-[var(--border-card)]">
            ...
            <BookMarked className="w-5 h-5 text-[var(--accent-text)]" />
            <div>
              <h1 className="text-lg font-semibold text-[var(--text-heading)]">Coach Commentaire d'arrêt</h1>
              <p className="text-xs text-[var(--text-muted)]">
                Rédige l'introduction et le plan d'un commentaire — correction sur 5 axes.
              </p>
            </div>
          </div>
      ...
      <div className="max-w-lg w-full space-y-6">
        <div className="text-center">
          ...
          <p className="text-sm text-[var(--text-muted)]">
            Une décision réelle sera tirée au hasard du corpus Cour de cassation. Rédige l'intro et le plan.
          </p>
        </div>
...
interface HistoryListProps {
  history: ReturnType<typeof useCommentaireCoach>['history']
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-1">
        {history.length === 0 ? (
          <p className="text-xs text-[var(--text-muted)] text-center mt-6 px-3">
            Pas encore de commentaires. Tire ta première décision.
          </p>
        ) : (
          history.map(h => {
