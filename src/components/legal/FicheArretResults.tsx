/**
 * Fiche d'arrêt grading results — 5-axis rubric + the decision re-displayed
 * for re-reading. No "model fiche" (the decision is real; no single canonical fiche).
 */
...
export function FicheArretResults({ task, submission, grading, onRetry, onNewDecision }: Props) {
  ...
  return (
    <div className="max-w-4xl mx-auto w-full flex flex-col gap-4 p-4">
      ...
      {/* Your fiche recap */}
      <FicheRecap submission={submission} />

      {/* Decision text for re-reading */}
...
function FicheRecap({ submission }: { submission: FicheSubmission }) {
  const [open, setOpen] = useState(false)
  const sections: Array<{ label: string; content: string }> = [
    { label: 'Faits', content: submission.faits || '(vide)' },
    { label: 'Procédure', content: submission.procedure || '(vide)' },
    { label: 'Moyens du pourvoi', content: submission.moyens || '(vide)' },
    { label: 'Question de droit', content: submission.questionDeDroit || '(vide)' },
    { label: 'Solution et portée', content: submission.solutionEtPortee || '(vide)' },
  ]
  return (
    <div className="glass-card overflow-hidden">
      ...
      >
        <span className="text-sm font-semibold text-[var(--text-heading)]">Ta fiche</span>
...
function DecisionReRead({ task }: { task: FicheTask }) {
  const [open, setOpen] = useState(false)
