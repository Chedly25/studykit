...
export function LevelsView({ subjects, chapters, topics, exerciseStatsByTopic, getChaptersForSubject, getTopicsForChapter }: LevelsViewProps) {
                  ...
                  return (
                    <div key={chapter.id}>
                      ...
                      {chapterExpanded && chapterTopics.map(topic => {
                        ...
                        return (
                          <div key={topic.id} className="flex items-center gap-3 px-10 py-2.5 border-b border-[var(--border-card)]/30 hover:bg-[var(--bg-input)]/20">
                            <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${masteryColor(topic.mastery)}`} />
                            <span className="flex-1 text-xs text-[var(--text-body)]">{topic.name}</span>
                            {stats && stats.total > 0 && (
                            ...
                            <Link
                              to={`/session?topic=${encodeURIComponent(topic.name)}`}
