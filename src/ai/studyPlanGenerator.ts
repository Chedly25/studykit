...
/** Streaming variant — emits days one-by-one via onDay callback. */
export async function generateStudyPlanDraftStreaming(
  examProfileId: string,
  authToken: string,
  daysAhead = 7,
  overrides?: PlanOverrides,
  onDay?: (day: ParsedPlanData['days'][number], index: number) => void,
...
- For days close to the exam, increase practice exams`
  ...
  const extractor = createStreamExtractor<ParsedPlanData['days'][number]>('days', {
    onItem: (day, index) => onDay?.(day, index),
  })
