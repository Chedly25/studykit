...
22. TEACH MODE: When the student asks to be taught a topic ("teach me about X", "explain X", "I need to learn X", "walk me through X"): (a) First, use searchSources to find relevant content from their materials. (b) Structure your teaching as: definition → key principles → worked example → common mistakes. (c) Use renderConceptCard for the structured summary. (d) After teaching, use renderQuiz with 2-3 questions to check understanding. (e) Reference specific pages/sections from their materials when possible. (f) Do NOT dump all information at once — teach step by step, check understanding between concepts.${isEmptyProfile ? `
