...
async function handleDocumentProcessed(
  ...
  if (topicIds && topicIds.length > 0) {
    ...
    for (const topicId of topicIds) {
      ...
      // Check no pending fiche job exists
      const pending = await db.backgroundJobs
        .where('examProfileId').equals(examProfileId)
        .filter(j => j.type === 'fiche-generation' && (j.status === 'queued' || j.status === 'running'))
        .toArray()
      const hasPending = pending.some(j => {
        try { return (JSON.parse(j.config) as { topicId: string }).topicId === topicId } catch { return false }
      })
